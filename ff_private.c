...
// RJVB (AV_LOG_VERBOSE)
#define FEEDBACKLEVEL	av_log_get_level()
...
{ double fTimebase, nearest_ntsc;
  const double small_interval = 1./120.;
	...
	fTimebase = av_q2d(*base), nearest_ntsc = floor(fTimebase * 1001. + .5) / 1001.;
...
void set_track_clean_aperture_ext(ImageDescriptionHandle imgDesc, Fixed displayW, Fixed displayH, Fixed pixelW, Fixed pixelH)
{
	...
	{ AVRational dar = {displayW, displayH},
		invPixelSize   = {pixelH, pixelW},
		...
		{ PixelAspectRatioImageDescriptionExtension **pasp = (PixelAspectRatioImageDescriptionExtension**)NewHandle(sizeof(PixelAspectRatioImageDescriptionExtension));
			...
			(*pasp)->hSpacing = EndianU32_NtoB(sar.num), (*pasp)->vSpacing = EndianU32_NtoB(sar.den);
...
void set_track_colorspace_ext(ImageDescriptionHandle imgDescHandle, Fixed displayW, Fixed displayH)
{
	...
	AVRational palRatio = {5, 4}, displayRatio = {displayW, displayH};
	...
	nclc = (NCLCColorInfoImageDescriptionExtension**)NewHandle(sizeof(NCLCColorInfoImageDescriptionExtension));
	...
	(*nclc)->colorParamType = EndianU32_NtoB(kVideoColorInfoImageDescriptionExtensionType),
		(*nclc)->primaries = EndianU16_NtoB(colorPrimaries),
		(*nclc)->transferFunction = EndianU16_NtoB(transferFunction),
		(*nclc)->matrix = EndianU16_NtoB(yuvMatrix);
...
//RJVB
#ifndef FFUSION_CODEC_ONLY
/* This function prepares the movie to receive the movie data,
	...
	for(j = 0; j < ic->nb_streams; j++) {
		...
		for(k = 0; k < stream->nb_index_entries; k++) {
			...
			if(codec->codec_type == CODEC_TYPE_VIDEO) {
				...
				av_log( NULL, FEEDBACKLEVEL,
					   "import_using_index read frame #%d at position %lld, size %d, duration %lld\n",
					   k, offset, stream->index_entries[k].size,
					   duration );
			}
// RJVB
#ifndef FFUSION_CODEC_ONLY
...
ComponentResult import_with_idle(ff_global_ptr storage, long inFlags, long *outFlags, int minFrames, int maxFrames, bool addSamples) {
	...
	while((readResult = av_read_frame(formatContext, &packet)) == 0) {
		...
		sampleRec.sampleFlags = flags;
		av_log( NULL, FEEDBACKLEVEL,
			   "import_with_idle read frame #%d at position %lld, size %d, duration %d (%llu dts)\n",
			   framesProcessed, packet.pos, packet.size,
			   packet.duration, (packet.dts - ncstream->lastdts) );
